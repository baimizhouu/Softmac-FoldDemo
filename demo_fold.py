...
JOINTNUM = 22 #The number of joints of the allegro hand.
indexJoint = [2, 3, 4, 5]
middleJoint = [7, 8, 9, 10]
ringJoint = [12, 13, 14, 15]
firstBatch = [2]
secondBatch = [4, 9, 14]
thirdBatch = [5, 10, 15]
dur1 = 10
...
class Controller:
    def __init__(
        ...
        self.actions = torch.zeros(steps, JOINTNUM, requires_grad=True)
        #[2]
        #self.actions.data[0,2] = 0.1
        # for col in secondBatch:
        #     self.actions.data[dur1:dur2+dur1,col] = 0.008
        # for col in thirdBatch:
        #     self.actions.data[dur1+dur2:,col] = 0.03
        # else:
    ...
    def step(self, grad):
        ...
        if grad.shape[0] > self.steps:
            grad = grad.reshape(self.steps, -1, JOINTNUM).mean(axis=1)
        actions_grad = grad
...
def get_init_actions(args, env, choice=0):
    if choice == 0:
        actions = torch.zeros(args.steps, JOINTNUM)
    elif choice == 1:
        actions = torch.ones(args.steps, JOINTNUM) * torch.tensor([1.0, -1.0]) * 1.2
    elif choice == 2:
        actions = torch.ones(args.steps, JOINTNUM) * torch.tensor([1.0, -1.0]) * 0.3
    else:
        assert False
...
def main(args):
    ...
    controller = Controller(
        steps=args.steps, substeps=args.steps, actions_init=actions,
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.render_interval == 0 or epoch == 0:
            images = render(env, n_steps=args.steps, interval=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp-name", "-n", type=str, default="fold")
    parser.add_argument("--config", type=str, default="config/demo_fold_config.py")
